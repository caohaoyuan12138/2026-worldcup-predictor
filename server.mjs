...
const engine = await import(new URL('model/_fusion_v5.mjs', import.meta.url).href);
...
function ensureElo() {
  ...
  // 确保 eloHistory 存在
  if (!dbData.eloHistory) dbData.eloHistory = [];
...
/**
 * 记录当前所有球队 Elo 快照到 eloHistory
 * 用于前端绘制 Elo 趋势图
 */
function recordEloSnapshot() {
  const snapshot = {
    timestamp: new Date().toISOString(),
    matchCount: dbData.completedMatches.filter(m => m.score).length,
    ratings: {}
  };
  for (const [name, t] of Object.entries(dbData.teams)) {
    snapshot.ratings[name] = t.eloRating || engine.rankToElo(t.rank || 50);
  }
  dbData.eloHistory.push(snapshot);
  // 最多保留 200 条快照
  if (dbData.eloHistory.length > 200) {
    dbData.eloHistory = dbData.eloHistory.slice(-200);
  }
}
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    if (pathname === '/api/matches/result' && method === 'POST') {
      ...
      recordEloSnapshot();
      saveDB();
      broadcastSSE('match_update', { type: 'result_added', match: newMatch });
    ...
    if (pathname.startsWith('/api/matches/') && method === 'DELETE') {
      ...
      ensureElo();
      saveDB();
      broadcastSSE('match_update', { type: 'result_deleted' });
    ...
    if (pathname === '/api/export/csv' && method === 'GET') {
      const type = queryParams.type || 'matches';
      let csv = '';
      if (type === 'matches') {
        csv = '日期,小组,主队,客队,比分,轮次\n';
        for (const m of (dbData.completedMatches || [])) {
          const g = m.group || dbData.teamGroup[m.home] || '';
          csv += `${m.date},${g},${m.home},${m.away},${m.score},${m.round || ''}\n`;
        }
      } else if (type === 'elo') {
        ensureElo();
        csv = '排名,球队,小组,Elo等级分,FIFA排名\n';
        const list = Object.entries(dbData.teams)
          .map(([n, d]) => ({ n, elo: d.eloRating || engine.rankToElo(d.rank || 50), rank: d.rank, g: dbData.teamGroup[n] || '' }))
          .sort((a, b) => b.elo - a.elo);
        list.forEach((t, i) => { csv += `${i+1},${t.n},${t.g},${t.elo},${t.rank || ''}\n`; });
      } else if (type === 'standings') {
        const standings = getStandings();
        csv = '小组,球队,场次,胜,平,负,进球,失球,净胜球,积分\n';
        for (const [name, s] of Object.entries(standings)) {
          if (!s.group) continue;
          csv += `${s.group},${name},${s.mp},${s.w},${s.d},${s.l},${s.gf},${s.ga},${s.gd},${s.p}\n`;
        }
      }
      res.writeHead(200, {
        'Content-Type': 'text/csv; charset=utf-8',
        'Content-Disposition': `attachment; filename="worldcup_${type}_${new Date().toISOString().slice(0,10)}.csv"`,
      });
      res.end('﻿' + csv);
      return;
    }
    ...
    if (pathname === '/api/dashboard' && method === 'GET') {
      ensureElo();
      const matches = dbData.completedMatches || [];
      const config = dbData.modelConfig || {};
      const weights = config.fusionWeights || { elo: 0.25, poisson: 0.30, economic: 0.10, market: 0.35 };

      // 方向准确率统计
      let correct = 0, wrong = 0, total = 0, exact = 0, top3Hit = 0;
      const roundAccuracy = {};
      const dailyAccuracy = {};
      ...
      for (const m of matches) {
        ...
        // 用简化 Elo 判断方向作为基线
        const tHome = dbData.teams[m.home];
        const tAway = dbData.teams[m.away];
        if (!tHome || !tAway) continue;
        const eloH = tHome.eloRating || engine.rankToElo(tHome.rank || 50);
        const eloA = tAway.eloRating || engine.rankToElo(tAway.rank || 50);
        const expH = engine.eloExpected(eloH, eloA);
        const predDir = expH > 0.45 ? 'home' : expH < 0.35 ? 'away' : 'draw';
        ...
        const top5 = [`${Math.round(expH * 3)}-0`, `${Math.round(expH * 2)}-1`, `${Math.round(expH * 1)}-0`, `1-${Math.round((1 - expH) * 2)}`, `0-0`];
        if (top5.slice(0, 3).includes(scoreStr)) top3Hit++;

        // 按轮次聚合
        const r = m.round || 'unknown';
        if (!roundAccuracy[r]) roundAccuracy[r] = { total: 0, correct: 0 };
        roundAccuracy[r].total++;
        if (predDir === actualDir) roundAccuracy[r].correct++;
        ...
        if (predDir === actualDir) dailyAccuracy[day].correct++;
      }

      // 预测日志统计
      const predLogs = dbData.predictionLog || [];
      const predDistribution = { home: 0, draw: 0, away: 0 };
      for (const p of predLogs) {
        if (p.prediction?.fusion?.winPct > p.prediction?.fusion?.awayPct && p.prediction?.fusion?.winPct > p.prediction?.fusion?.drawPct) predDistribution.home++;
        else if (p.prediction?.fusion?.drawPct > p.prediction?.fusion?.winPct) predDistribution.draw++;
        else predDistribution.away++;
      }

      sendJSON(res, {
        stats: {
          totalMatches: matches.filter(m => m.score).length,
          totalPredictions: total,
          directionAccuracy: total > 0 ? +(correct / total * 100).toFixed(1) : 0,
          correctCount: correct,
          wrongCount: wrong,
          top3HitRate: total > 0 ? +(top3Hit / total * 100).toFixed(1) : 0,
          exactMatchCount: exact,
        },
        roundAccuracy: Object.entries(roundAccuracy)
          .map(([round, v]) => ({ round, accuracy: +(v.correct / v.total * 100).toFixed(1), total: v.total }))
          .sort((a, b) => a.round - b.round),
        dailyAccuracy: Object.entries(dailyAccuracy)
          .map(([date, v]) => ({ date, accuracy: +(v.correct / v.total * 100).toFixed(1), total: v.total }))
    ...
    // 16b. Elo 历史趋势 (用于折线图)
    // ========================================
    if (pathname === '/api/elo/history' && method === 'GET') {
      ensureElo();
      const teamNames = queryParams.teams ? queryParams.teams.split(',') : null;
      const history = (dbData.eloHistory || []).map(snapshot => ({
        timestamp: snapshot.timestamp,
        matchCount: snapshot.matchCount,
        ratings: teamNames
          ? Object.fromEntries(
              teamNames.filter(n => snapshot.ratings[n] != null).map(n => [n, snapshot.ratings[n]])
            )
          : snapshot.ratings
      }));
    ...
    if (pathname === '/api/knockout' && method === 'GET') {
      ...
      if (dbData.groupStandings) {
        for (const [g, info] of Object.entries(dbData.groupStandings)) {
          const teamList = info.teams || [];
          const gs = teamList.map(t => standings[t]).filter(Boolean)
            .sort((a, b) => b.p - a.p || b.gd - a.gd || b.gf - a.gf);
...
// ============================================================
// AI 推理裁判 (调用 Python 脚本 + agnes-2.0-flash)
// ============================================================

const SENSENOVA_CONFIG = {
  apiKey: 'sk-6FqQ8pmPLUuouABzdDihaUkUG730w7ADT6zxySDodQUFuGGe',
...
server.listen(PORT, '127.0.0.1', () => {
  console.log('');
  console.log('╔══════════════════════════════════════════════╗');
  console.log('║  ⚽ 世界杯预测系统 v5.0 - Oracle V2 五层架构');
  console.log('╚══════════════════════════════════════════════╝');
  console.log('');
