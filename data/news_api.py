...
# SportsMole 本地 API（由 start_services.py 启动）
SPORTSMOLE_API = "http://localhost:3000"
...
def get_sportsmole_previews(date_filter: str = None) -> Dict:
    """
    从本地 SportsMole API 获取比赛预告数据

    Args:
        date_filter: 日期过滤，如 "2026-06-23"

    Returns:
        {
            "count": int,
            "matches": [{"homeTeam": "...", "awayTeam": "...", "time": "...", "link": "...", "date": "..."}],
            "groups": [{"date": "...", "league": "...", "matches": [...]}]
        }
    """
    try:
        url = f"{SPORTSMOLE_API}/api/previews"
        if date_filter:
        ...
        if r.status_code == 200:
            ...
            return {"error": f"HTTP {r.status_code}", "count": 0, "matches": [], "groups": []}
    except requests.exceptions.ConnectionError:
        return {"error": "SportsMole API not running", "count": 0, "matches": [], "groups": []}
    except Exception as e:
        return {"error": str(e)[:100], "count": 0, "matches": [], "groups": []}


def get_sportsmole_match_preview(url_or_path: str) -> Dict:
    """
    从本地 SportsMole API 获取单场比赛详细预告

    Args:
        url_or_path: 比赛 URL 或路径

    Returns:
        {"title": "...", "content": "...", "teams": [...], "url": "..."}
    """
    try:
        # 处理完整 URL 或相对路径
        ...
        r = requests.get(f"{SPORTSMOLE_API}/api/previews/{path.lstrip('/')}", timeout=15)
        if r.status_code == 200:
            return r.json()
        else:
            return {"error": f"HTTP {r.status_code}"}
    except requests.exceptions.ConnectionError:
        return {"error": "SportsMole API not running"}
    except Exception as e:
        return {"error": str(e)[:100]}


def get_sportsmole_news_for_match(home_team: str, away_team: str) -> str:
    """
    获取 SportsMole 比赛预告信息，格式化为预测输入
    ...
    previews = get_sportsmole_previews(date_filter=today)

    if "error" in previews:
        return f"SportsMole API 不可用: {previews['error']}"

    # 中文名称 -> 英文名称映射（反向）
    CN_TO_EN = {v: k for k, v in TEAM_NAME_MAP.items()}

    home_en = TEAM_NAME_MAP.get(home_team, home_team).lower()
    away_en = TEAM_NAME_MAP.get(away_team, away_team).lower()

    # 查找匹配的比赛
    for match in previews.get("matches", []):
        match_home = match.get("homeTeam", "").lower()
        match_away = match.get("awayTeam", "").lower()

        if (home_en in match_home or match_home in home_en) and \
           (away_en in match_away or match_away in match_away):
            # 找到匹配的比赛
            title = match.get("title", "")
            link = match.get("link", "")
            return f"📰 SportsMole 预告: {title}\n🔗 {link}"

    # 如果没找到具体比赛，返回所有今日比赛
    if previews.get("count", 0) > 0:
        lines = [f"📰 SportsMole 今日比赛预告 ({previews['count']}场):"]
        for m in prevs.get("matches", [])[:5]:
            lines.append(f"  • {m.get('title', '未知')}")
        return "\n".join(lines)

    return "暂无 SportsMole 比赛预告"